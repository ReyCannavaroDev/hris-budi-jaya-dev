...
async function tipeGrade(grading_id) {
  if (!grading_id || !grading_id.id) {
    // Reset values
    values.tipe_grade = null;
    return;
  }
  try {
    const dataURL = `${store.server.url_backend}/operation/m_general/${grading_id.id}`;
    const params = {
      join: false,
      transform: false,
    };
    ...
    console.log('CEK DATA GRADING', initialValues)
...
async function onSave() {
  ...
  try {
    ...
    //   })
    //   formErrors.value['kode'] = ['Nik Tidak Boleh Kosong']
    //   isNext = false
    // }else{
    //   delete formErrors.value['kode']
    // }
    // if(!isNext) return
    ...
    //   })
    //   formErrors.value['nama_depan'] = ['Nama Depan Tidak Boleh Kosong']
    //   isNext = false
    // }else{
    //   delete formErrors.value['nama_depan']
    // }
    // if(!isNext) return
...
const landing = reactive({
  ...
  api: {
    ...
    params: {
      simplest: true,
      searchfield: ' kode, atasan.nama_lengkap, nama_lengkap, m_dir.nama, m_dept.nama, alamat_domisili, no_tlp',
    },
    onsuccess(response) {
